...
(() => {
 ...
 function embed(host,tab,hero=false){
  ...
  const frame=document.createElement('iframe');frame.title=hero?'Interactive Sharpe portfolio — fictional sample data':'Interactive Sharpe dashboard — fictional sample data';frame.setAttribute('sandbox','allow-scripts allow-same-origin allow-downloads');frame.loading=hero?'eager':'lazy';frame.src='/demo/?tab='+tab;
  ...
  const full=document.createElement('a');full.href='/demo/?tab='+tab;full.target='_blank';full.rel='noopener';full.textContent='Open full-size demo';caption.append(full);shell.append(caption);host.append(shell);
  function resize(){const w=stage.clientWidth,base=Math.max(1440,w),scale=w/base,h=hero?960:1120;frame.style.width=base+'px';frame.style.height=h+'px';frame.style.transform=`scale(${scale})`;stage.style.height=h*scale+'px';}
  new ResizeObserver(resize).observe(stage);resize();
  const entry={frame,full,tab,hero,ready:false};frames.push(entry);return entry;
 }
 const hero=document.querySelector('.hero-product');if(hero){const original=[...hero.children];const entry=embed(hero,'portfolio',true);entry.fallback=()=>original.forEach(el=>el.hidden=true);}
 ...
 if(panels){feature=embed(panels,'portfolio');feature.fallback=()=>panels.querySelectorAll('.feature-panel figure').forEach(el=>el.hidden=true);
  document.querySelectorAll('[data-feature]').forEach(button=>button.addEventListener('click',()=>{feature.tab=button.dataset.feature;feature.full.href='/demo/?tab='+feature.tab;if(feature.ready)feature.frame.contentWindow.postMessage({type:'sharpe-demo-tab',tab:feature.tab},location.origin);}));
 }
